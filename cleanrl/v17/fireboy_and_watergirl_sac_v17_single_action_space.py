...
# v17 New smaller CNN SINGLE ACTION SPACE
...
class FireboyAndWatergirlEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.steps = 0
        self.max_steps = 128 * 4
        self.envs = 8

        self.level_height = 20 - 2  # Assuming 1-tile border on top and bottom
        self.level_width = 20 - 2   # Assuming 1-tile border on left and right
        ...
        self.observation_space = spaces.Box(
            low=0,
            high=17,  # 17 different tile types
    ...
    def _load_level(self):
        ...
        # self.fire_boy = FireBoy((random.randint(1, 17) * 16, 17 * 16))
        # self.water_girl = WaterGirl((random.randint(1, 17) * 16, 17 * 16))

        # self.doors.append(FireDoor((random.randint(1, 19) * 16, 17 * 16)))
        # self.doors.append(WaterDoor((random.randint(1, 19) * 16, 17 * 16)))
        # self.stars.append(Star([random.randint(1, 19) * 16, 17 * 16], "fire"))
        # self.stars.append(Star([random.randint(1, 19) * 16, 17 * 16], "water"))
    ...
    def step(self, action):
        ...
        rgb_img = self.state_to_rgb(self.state)
    ...
    def state_to_rgb(self, state):
        """
        Convert a tile index map (state) to an RGB image using color_mapping.
        """
        color_mapping = {
            0: [255, 255, 255],
            1: [50, 50, 50],
    ...
    def _get_state(self):
        ...
        image = np.zeros((inner_height, inner_width), dtype=np.uint8)
        tile_map = {
            ' ': 0,
            'S': 1,
            'L': 2,
        ...
        for tile_char, color in tile_map.items():
        ...
        for (fb_x, fb_y) in self.fb_visited_positions:
            if 0 <= fb_y < image.shape[0] and 0 <= fb_x < image.shape[1]:
                # Only color visited tile if it's not water, fire, or goo
                if not (
                    np.array_equal(image[fb_y, fb_x], tile_map['W']) or
                    np.array_equal(image[fb_y, fb_x], tile_map['L']) or
                    np.array_equal(image[fb_y, fb_x], tile_map['G'])
                ):
                    image[fb_y, fb_x] = light_red
        for (wg_x, wg_y) in self.wg_visited_positions:
            if 0 <= wg_y < image.shape[0] and 0 <= wg_x < image.shape[1]:
                if not (
                    np.array_equal(image[wg_y, wg_x], tile_map['W']) or
                    np.array_equal(image[wg_y, wg_x], tile_map['L']) or
                    np.array_equal(image[wg_y, wg_x], tile_map['G'])
                ):
                    image[wg_y, wg_x] = light_blue

        # Draw stars (vectorized)
        for star in self.stars:
            s_x, s_y = np.array(star.get_position()) // 16 - border_size
            if 0 <= s_y < image.shape[0] and 0 <= s_x < image.shape[1]:
                if not star.is_collected:
                    image[s_y, s_x] = tile_map['a' if star._player ==
                                               "fire" else 'b']
                else:
                    image[s_y, s_x] = tile_map[' ']
        ...
        for door in self.doors:
            ...
            if 0 <= d_y < image.shape[0] and 0 <= d_x < image.shape[1]:
                # if not door.reward_given:
                image[d_y][d_x] = tile_map['A' if door._player ==
                                           "fire" else 'B']
                # else:
    ...
    def _compute_reward(self):
        ...
        if self.fire_boy:
            if 0 <= fb_y < self.level_height and 0 <= fb_x < self.level_width:
                ...
                if tile_fb == 'W':  # Water or Goo
                    ...
                    fb_reward += false_liquid_punishment
                elif tile_fb == 'G':
                    self.times_in_goo += 1
                    fb_reward += false_liquid_punishment
                else:
        ...
        if self.water_girl:
            if 0 <= wg_y < self.level_height and 0 <= wg_x < self.level_width:
                ...
                if tile_wg == 'L':  # Fire or Goo
                    ...
                    wg_reward += false_liquid_punishment
                elif tile_wg == 'G':
                    self.times_in_goo += 1
                    wg_reward += false_liquid_punishment
                else:
        ...
        fb_reward += new_fb_positions*exploration_reward
        wg_reward += new_wg_positions*exploration_reward
        ...
        for i, star in enumerate(stars):
            if is_collected[i] and not reward_given[i]:
                if star._player == 'fire':
                    fb_reward += star_reward
                elif star._player == 'water':
                    wg_reward += star_reward
        ...
        if all(player_at_door) and not all(reward_given):
            fb_reward += finish_reward
            wg_reward += finish_reward
            for door in doors:
                door.reward_given = True
    ...
    def _check_player_dead(self):
        ...
        return fireboy_dead or watergirl_dead


register(
    id="FireboyAndWatergirl-sac-v17-single-action-space",
    entry_point="cleanrl.v17.fireboy_and_watergirl_sac_v17_single_action_space:FireboyAndWatergirlEnv",
)

...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        num_tile_types = 18  # envs.single_observation_space.high.max() + 1  # e.g., 18
        # 8 is embedding dim, tune as needed
        self.embedding = nn.Embedding(num_tile_types, 8)
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            # 18x18 -> 4x4 after 3 convs with stride
            layer_init(nn.Linear(128 * 1 * 1, 512)),
            nn.ReLU(),
        )
        self.actor = layer_init(
            nn.Linear(512, 8), std=0.01)
        self.critic = layer_init(nn.Linear(512, 1), std=1)

    def preprocess(self, x):
        # x shape: (batch, 4, 18, 18), integer tile indices
        x = self.embedding(x.long())  # (batch, 4, 18, 18, 8)
        x = x.permute(0, 1, 4, 2, 3).reshape(
            x.shape[0], -1, x.shape[2], x.shape[3])  # (batch, 32, 18, 18)
        return x

...
class FireboyAndWatergirlEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # Define the obsdservation space (feature-based representation)
        # [Fireboy_x, Fireboy_y, Watergirl_x, Watergirl_y, Gate_status, Door_status]
        # self.obssdervation_space = spaces.Box(
        ...
        self.observation_space = spaces.Box(
            low=0,
            high=255,
            shape=(level_width*level_height,),  # Example: (34, 25, 1)
            dtype=np.float32
        )
    ...
    def _get_state(self):
        ...
        # Process to ensure exactly 2550 elements
        # Option 1: Create an image with 3 channels (like RGB)
        # 850×3=2550, so create a 3-channel image from your data
        # This might work better with neural networks that expect image data
        resized_grid = cv2.resize(
            inner_grid, (34, 25), interpolation=cv2.INTER_NEAREST)  # 850 elements

        # Create a 3-channel image (850×3=2550)
        three_channel_grid = np.stack(
            [resized_grid], axis=-1)

        # Flatten to get exactly 2550 elements
        flat_obs = three_channel_grid.flatten().astype(np.float32)
...
register(
    id="FireboyAndWatergirl-dqn-v0",  # Unique ID for the environment
    # Path to the environment class
    entry_point="cleanrl.fireboy_and_wategirl_dqn:FireboyAndWatergirlEnv",
)

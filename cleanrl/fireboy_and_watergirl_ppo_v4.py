...
class FireboyAndWatergirlEnv(gym.Env):
    ...
    def _load_level(self):
        ...
        # self.fire_boy = FireBoy((200, 336))
        # self.water_girl = WaterGirl((200, 336))
        ...
        for y, row in enumerate(level_data):
            for x, tile in enumerate(row):
                ...
                elif tile == 'f':  # Fireboy starting position
                    self.fire_boy = FireBoy((x * 16, y * 16))
                elif tile == 'w':  # Watergirl starting position
                    self.water_girl = WaterGirl((x * 16, y * 16))
                # elif tile == 'D':  # Gate
    ...
    def _get_state(self, draw=False):
        ...
        for y in range(border_size, len(level_data) - border_size):
            for x in range(border_size, len(level_data[0]) - border_size):
                ...
                tile = color_mapping.get(
                    level_data[y][x], [100, 100, 100])  # Default gray for unknown tiles

                # don't draw the initial position of the characters, because they can move
                if tile == color_mapping['f']:
                    tile = color_mapping[' ']
                if tile == color_mapping['w']:
                    tile = color_mapping[' ']
                rgb_image[inner_y, inner_x] = tile
    ...
    def _check_done(self):
        ...
        if all(star.is_collected for star in self.stars):
            return True
        return False

...
class FireboyAndWatergirlEnv(gym.Env):
    ...
    def _apply_action(self, action):
        ...
        """
        # Extract the scalar value from the action
        if isinstance(action, np.ndarray):
            action_value = action.item()
        else:
            action_value = action

        # Map from [-1,1] to [0,7] by dividing into 8 equal segments
        # Each segment has a width of 0.25
        # [-1.0, -0.75) -> 0
        # [-0.75, -0.5) -> 1
        # [-0.5, -0.25) -> 2
        # [-0.25, 0.0) -> 3
        ...
        discrete_action = min(7, max(0, int((action_value + 1) * 4)))

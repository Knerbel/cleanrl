...
class FireboyAndWatergirlEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.fireboy_walk_left = 0
        self.fireboy_walk_right = 0
        self.steps = 0
        self.max_steps = 400
    ...
    def reset(self, seed=None, options=None):
        ...
        self.done = False
        self.steps = 0
        print(self.fireboy_walk_left, self.fireboy_walk_right)
        self.fireboy_walk_left = 0
        self.fireboy_walk_right = 0
    ...
    def _apply_action(self, action):
        ...
        if action < 0:
            fireboy_action = int(
                np.clip(np.abs(action), 0, 3))  # Map to [0, 3]
            watergirl_action = 3  # Default action for Watergirl
        elif action > 0:
            watergirl_action = int(np.clip(action, 0, 3))  # Map to [0, 3]
            fireboy_action = 3  # Default action for Fireboy
        else:
            fireboy_action = 3  # Default action for Fireboy
            watergirl_action = 3  # Default action for Watergirl
        ...
        if self.fire_boy.moving_right:
            self.fireboy_walk_right += 1
        if self.fire_boy.moving_left:
            self.fireboy_walk_left += 1
...
register(
    id="FireboyAndWatergirl-rpo-v0",  # Unique ID for the environment
    # Path to the environment class
    entry_point="cleanrl.fireboy_and_watergirl_rpo:FireboyAndWatergirlEnv",
)

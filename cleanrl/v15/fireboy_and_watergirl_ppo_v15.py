...
# v15 Generalization two, new model
...
class FireboyAndWatergirlEnv(gym.Env):
    ...
    def __init__(self):
        ...
        img_shape = (self.level_height, self.level_width, self.num_channels)
        img_size = int(np.prod(img_shape))
        # [fb_x, fb_y, wg_x, wg_y, firedoor_x, firedoor_y, waterdoor_x, waterdoor_y]
        pos_size = 8

        self.observation_space = spaces.Box(
            low=0,
            high=255,
            shape=(img_size + pos_size,),
            dtype=np.float32
        )
    ...
    def _load_level(self):
        ...
        #     ['level6_generalization'])
        self.level = ['level7a', 'level7b'][random.randint(0, 1)]
    ...
    def _get_state(self):
        ...
        for (fb_x, fb_y) in self.fb_visited_positions:
            if 0 <= fb_y < rgb_image.shape[0] and 0 <= fb_x < rgb_image.shape[1]:
                ...
                if not (
                    np.array_equal(rgb_image[fb_y, fb_x], color_mapping['W']) or
                    np.array_equal(rgb_image[fb_y, fb_x], color_mapping['L']) or
                    np.array_equal(rgb_image[fb_y, fb_x], color_mapping['G'])
                ):
        ...
        for (wg_x, wg_y) in self.wg_visited_positions:
            if 0 <= wg_y < rgb_image.shape[0] and 0 <= wg_x < rgb_image.shape[1]:
                if not (
                    np.array_equal(rgb_image[wg_y, wg_x], color_mapping['W']) or
                    np.array_equal(rgb_image[wg_y, wg_x], color_mapping['L']) or
                    np.array_equal(rgb_image[wg_y, wg_x], color_mapping['G'])
                ):
        ...
        firedoor_x = firedoor_y = waterdoor_x = waterdoor_y = 0
        for door in self.doors:
            d_x, d_y = np.array(door.get_position()) // 16
            d_x, d_y = int(d_x-1), int(d_y-1)
            if door._player == "fire":
                firedoor_x, firedoor_y = d_x, d_y
            elif door._player == "water":
                waterdoor_x, waterdoor_y = d_x, d_y

        positions = np.array([fb_x, fb_y, wg_x, wg_y, firedoor_x,
                             firedoor_y, waterdoor_x, waterdoor_y], dtype=np.int32)

        img_flat = rgb_image.astype(np.float32).flatten()
        pos_flat = positions.astype(np.float32).flatten()
        return np.concatenate([img_flat, pos_flat])
...
register(
    id="FireboyAndWatergirl-ppo-v15",
    entry_point="cleanrl.v15.fireboy_and_watergirl_ppo_v15:FireboyAndWatergirlEnv",
)

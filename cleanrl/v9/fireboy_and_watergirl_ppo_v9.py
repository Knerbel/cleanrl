...
class FireboyAndWatergirlEnv(gym.Env):
    ...
    def _compute_reward(self):
        ...
        reward += fb_movement * 1  # 5 points per tile moved right
        reward += wg_movement * 1
        ...
        # if fb_x >= self.level_width - 2 and wg_x >= self.level_width - 2:
    ...
    def _check_done(self):
        ...
        wg_x, wg_y = np.array(self.water_girl.get_position()) // 16
        wg_x = int(wg_x-1)
        return fb_x >= self.level_width - 2 and wg_x >= self.level_width - 2

...
from heatmaps.heatmap import create_heatmap, create_legend
...
if __name__ == "__main__":
    ...
    #     run_name="Random_obstacles",
    #     epsilon=1,
    #     eval_episodes=100,
    # )
    # create_heatmap(f"{final_runs}", name="Random_obstacles")
    # clear_image_folder(final_runs)

    # evaluate(
    #     model_path=f"{training_runs}level8_exploration\\PPO\\PPO_best_model.pt",
    #     env_id="FireboyAndWatergirl-ppo-v17-stars",
    #     run_name="Random_stars",
    #     epsilon=1,
    #     eval_episodes=100,
    # )
    # create_heatmap(f"{final_runs}", name="Random_stars")
    # clear_image_folder(final_runs)

    evaluate(
        model_path=f"{training_runs}level8_exploration\\PPO\\PPO_best_model.pt",
        env_id="FireboyAndWatergirl-ppo-v17-plates_and_gates",
        run_name="Random_plates-and-gates",
        epsilon=1,
        eval_episodes=100,
    )
    create_heatmap(f"{final_runs}", name="Random_plates_and_gates")
    clear_image_folder(final_runs)

    evaluate(
        model_path=f"{training_runs}level8_exploration\\PPO\\PPO_best_model.pt",
        env_id="FireboyAndWatergirl-ppo-v17-combined",
        run_name="Random_combined",
        epsilon=1,
        eval_episodes=100,
    )
    create_heatmap(f"{final_runs}", name="Random_combined")
    clear_image_folder(final_runs)
    # create_legend(
    #     f"{final_runs}", name="FOOBAR")
    ...
    # # )
    # # create_heatmap(f"{final_runs}",
    # #                name="PPO_pretrained_evaluation_combined_best_n_model")
    # # clear_image_folder(final_runs)
